...
def plot_envpartners_occurrences(dynophore, superfeature_name, max_frames=1000):
    ...
    # Prepare data
    occurrences = _prepare_plot_occurrences(dynophore.envpartners_occurrences[superfeature_name], max_frames)
...
def plot_superfeatures_vs_envpartners(dynophore):
    ...
    data = dynophore.frequency[dynophore.frequency.loc['any', :].sort_values(ascending=False).index]
...
def _prepare_plot_envpartners_distances(dynophore, superfeature_name, max_frames=1000):
    ...
    # Sort data by ratio
    ratio = dynophore.frequency.loc[:, superfeature_name].dropna().drop('any').sort_values(ascending=False)
